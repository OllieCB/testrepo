...
var defaults = {
    ...
    item: '<li role="option" aria-selected="false"></li>',
    minLength: 1,
    autoselect: true
}
...
var Typeahead = function (element, options) {
    ...
    self.inputField = element;
    self.element = dom(element);
    self.combobox = dom(element.parentElement);
...
proto.clearSelected = function() {
    var self = this;
    self.element.attr('aria-activedescendant', '');
    self.hasSelection = false;
}

proto.clearMenu = function() {
    var self = this;
    self.menu.empty;
    self.clearSelected();
}
...
proto.show = function () {
    ...
    self.shown = true;
    self.combobox.attr('aria-expanded', true);
    return self;
}
...
proto.render = function (items) {
    ...
    if (self.options.autoselect) {
        items[0].addClass('active').attr('aria-selected', true);
        self.element.attr('aria-activedescendant', items[0].attr('id'));
